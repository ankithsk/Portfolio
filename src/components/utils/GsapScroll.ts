...
export function setCharTimeline(
  ...
  character?.children.forEach((object: any) => {
    // Monitor setup — only if the model has a "Plane004" object (original model specific)
    ...
    // Screen light — only if model has it
  ...
  // Try multiple bone name conventions for neck
  ...
  if (window.innerWidth > 1024) {
    if (character) {
      ...
      // Neck bone animation — only if found
      if (neckBone) {
        tl2.to(neckBone.rotation, { x: 0.6, delay: 2, duration: 3 }, 0);
      }
      // Monitor/screenlight animations — only if model has these objects
      if (monitor) {
